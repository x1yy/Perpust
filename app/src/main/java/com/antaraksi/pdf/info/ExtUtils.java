...
import com.antaraksi.ext.CbzCbrExtractor;
import com.antaraksi.ext.Fb2Extractor;
...
import com.antaraksi.pdf.search.activity.HorizontalModeController;
...
import com.antaraksi.zipmanager.ZipDialog;
...
public class ExtUtils {
    ...
    public static String readableFileSize(long size) {
        if (true) {
            return Formatter.formatFileSize(PerpustApp.context, size).replace(" ", "");
        }
        if (size <= 0)
            return "0";
    ...
    public static void sharePage(final Activity a, final File file, int page, String pageUrl) {
        ...
        Glide.with(PerpustApp.context).asBitmap().load(pageUrl).into(new SimpleTarget<Bitmap>() {
            @Override
